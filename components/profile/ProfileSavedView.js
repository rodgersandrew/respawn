...
class ProfileSavedView extends Component {
  state = {
    loading: true,
    savedPosts: []
  };
  ...
  componentDidMount() {
    this.props.getUserSaved(this.props.user);
  }

  async componentWillReceiveProps(nextProps) {
    if (nextProps.savedPosts) {
      await this.generateThumbnailsFromPosts(nextProps.savedPosts);
  ...
  render() {
    ...
    return (
      <FlatList
        ...
        data={formatData(this.state.savedPosts, numColumns)}
  ...
  generateThumbnailsFromPosts = async posts => {
    ...
    this.setState({ savedPosts: postarray });
  };
}
...
export default connect(mapStateToProps, actions)(ProfileSavedView);
